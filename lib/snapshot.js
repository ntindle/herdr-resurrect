...
function buildPane(snap, paneId, index, layout) {
  ...
  if (p.agent) {
    ...
    out.agent = {
      ...
      session: p.agent_session || null, // native resume ref, when the integration reports one
    };
    ...
    .map((ws) => {
      const tabs = (snap.tabs || [])
        ...
        .map((t) => {
          ...
          const panes = order.map((pid, i) => buildPane(snap, pid, i, layout));
          return {
            tab_id: t.tab_id,
            number: t.number,
            label: t.label,
